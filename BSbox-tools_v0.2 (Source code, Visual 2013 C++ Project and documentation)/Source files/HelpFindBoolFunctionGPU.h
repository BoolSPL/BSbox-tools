...
void FindReadFromFileComputeBooleanGPU(string filename, int menuChoice, int nl_gpuBorder, int AC_gpuBorder, int deg_gpuBorder)
{
	...
	host_Bool_PTT = (int *)malloc(sizeof(int)* size);

	//check CUDA component status
	cudaError_t cudaStatus;

	//input device vector - boolean function
	cudaStatus = cudaMalloc((void **)&device_Bool_TT, sizeof(int)* size);
	...
	//input device vector - boolean function
	cudaStatus = cudaMalloc((void **)&device_Bool_PTT, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		//goto Error;
		exit(EXIT_FAILURE);
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 0) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					if (menuChoice == 0)
					{
						...
						int Lin_gpu = WalshSpecTranBoolGPU_ButterflyMax(device_Bool_PTT, device_Bool_rez, size, true); //use Butterfly Max
					...
					if (menuChoice == 1)
					{
						...
						int AC_gpu = AutocorrelationTranBoolGPU_ButterflyMax(device_Bool_PTT, device_Bool_rez, size, true); //use Butterfly Max
						/////////////////////////////////////////////////////////////////////////////
					...
					if (menuChoice == 2)
					{
						...
						int degMax_gpu = AlgebraicDegreeBoolGPU_ButterflyMax(device_Bool_TT, device_Bool_rez, size); //use Butterfly Max
						/////////////////////////////////////////////////////////////////////////////////////////////////
