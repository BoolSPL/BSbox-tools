...
void ReadFromFileComputeNlGPU(string filename, int menuChoice)
{
	...
	walshvec_gpu = (int *)malloc(sizeof(int)* size);

	//check CUDA component status
	cudaError_t cudaStatus;

	//input device vector - boolean function
	cudaStatus = cudaMalloc((void **)&device_Bool, sizeof(int)* size);
	...
	cudaStatus = cudaMalloc((void **)&device_Bool_rez, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		//goto Error;
		exit(EXIT_FAILURE);
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 0) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					if (sizeBool != size)
					{
						...
						walshvec_gpu = (int *)malloc(sizeof(int)* sizeBool);
						...
						cudaMalloc((void **)&device_Bool, sizeof(int)* sizeBool);
					...
					cudaStatus = cudaMemcpy(device_Bool, host_Bool_TT, sizeof(int)* sizeBool, cudaMemcpyHostToDevice);
					if (cudaStatus != cudaSuccess) {
					...
					//menuChoice for compute Walsh spectar
					if (menuChoice == 0)
					{				
						...
						cudaStatus = cudaMemcpy(walshvec_gpu, device_Bool_rez, sizeBool, cudaMemcpyDeviceToHost);
						if (cudaStatus != cudaSuccess) {
							fprintf(stderr, "cudaMemcpy failed!");
							//goto Error;
							exit(EXIT_FAILURE);
						...
						{
							myfile << walshvec_gpu[ii] << " ";
						}
						counterBoolean++;
					}

					//menuChoice for compute Lin
					if (menuChoice == 1)
					{
						...
						/////////////////////////////////////////////////////////////////////////////////////////////////
						//Function: Fast Walsh Transformation function GPU (W_f(f))
						int Lin_gpu = WalshSpecTranBoolGPU_ButterflyMax(device_Bool, device_Bool_rez, size, true); //use Butterfly Max
						/////////////////////////////////////////////////////////////////////////////////////////////////
					...
					//menuChoice for compute Walsh spectar and Lin
					if (menuChoice == 2)
					{
						//@@Start timer on kernel
						cudaEventRecord(start_gpu_all, 0);
		...
		while (fin >> str)
		{
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 0) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					//menuChoice for compute Autocorrelation spectar
					if (menuChoice == 0)
					{
						//@@Start timer on kernel
						cudaEventRecord(start_gpu_all, 0);
						...
						// Copy output vector from GPU buffer to host memory.
						cudaStatus = cudaMemcpy(rf_cpu, device_Bool_rez, sizeBool, cudaMemcpyDeviceToHost);
						if (cudaStatus != cudaSuccess) {
							fprintf(stderr, "cudaMemcpy failed!");
							//goto Error;
							exit(EXIT_FAILURE);
					...
					if (menuChoice == 1)
					{
						...
						int AC_gpu = AutocorrelationTranBoolGPU_ButterflyMax(device_Bool, device_Bool_rez, size, true); //use Butterfly Max
						/////////////////////////////////////////////////////////////////////////////////////////////////
					...
					//menuChoice for compute Autocorrelation spectar and AC
					if (menuChoice == 2)
					{
						//@@Start timer on kernel
						cudaEventRecord(start_gpu_all, 0);
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 0) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					if (menuChoice == 0)
					{
						...
						//////////////////////////////////////////////////////////////////////////////////////////////////
						MobiusTranBoolGPU(device_Bool, device_Bool_rez, size);
						//////////////////////////////////////////////////////////////////////////////////////////////////
						...
						// Copy output vector from GPU buffer to host memory.
						cudaStatus = cudaMemcpy(anf_cpu, device_Bool_rez, sizeBool, cudaMemcpyDeviceToHost);
						if (cudaStatus != cudaSuccess) {
							fprintf(stderr, "cudaMemcpy failed!");
							//goto Error;
							exit(EXIT_FAILURE);
					...
					if (menuChoice == 1)
					{
						...
						int degMax_gpu = AlgebraicDegreeBoolGPU_ButterflyMax(device_Bool, device_Bool_rez, size); //use Butterfly Max
						//////////////////////////////////////////////////////////////////////////////////////////////////
...
void ReadFromFileComputeBitWise_ANFGPU(string filename, int menuChoice)
{
	...
	host_NumIntVecTT = (unsigned long long int *)malloc(sizeof(unsigned long long int)* size);
	host_NumIntVecANF = (unsigned long long int *)malloc(sizeof(unsigned long long int)* size);

	host_max_values_AD = (int *)malloc(sizeof(int)* size);

	//check CUDA component status
	cudaError_t cudaStatus;

	//input integer TT device vector
	cudaStatus = cudaMalloc((void **)&device_NumIntVecTT, sizeof(unsigned long long int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		//goto Error;
		exit(EXIT_FAILURE);
	...
	cudaStatus = cudaMalloc((void **)&device_NumIntVecANF, sizeof(unsigned long long int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		//goto Error;
		exit(EXIT_FAILURE);
	...
	cudaStatus = cudaMalloc((void **)&device_max_values_AD, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		//goto Error;
		exit(EXIT_FAILURE);
		...
		while (fin >> str)
		{
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 0) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					BinVecToDec(NumOfBits, host_Bool_TT, host_NumIntVecTT, NumInt);
					...
					if (menuChoice == 0)
					{
						...
						DecVecToBin(NumOfBits, anf_cpu, host_NumIntVecANF, NumInt);
