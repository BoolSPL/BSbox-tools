...
void ReadFromFileComputeNlSboxGPU(string filename, int menuChoice)
{
	...
	LAT = (int *)malloc(sizeof(int)* size);

	//check CUDA component status
	cudaError_t cudaStatus;

	//input S-box device vector
	cudaStatus = cudaMalloc((void **)&device_Sbox, sizeof(int)* size);
	...
	cudaStatus = cudaMalloc((void **)&device_CF, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		exit(EXIT_FAILURE);
	}

	cudaStatus = cudaMalloc((void **)&device_LAT, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
	exit(EXIT_FAILURE);
	}
...
void ReadFromFileComputeACSboxGPU(string filename, int menuChoice)
{
	...
	//Declaration and Allocate memory blocks 
	ACT = (int *)malloc(sizeof(int)* size);
	...
	cudaStatus = cudaMalloc((void **)&device_ACT, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
			...
			if (str == ch)
			{
				...
				if ((parameter[0] == 1) & (parameter[1] > 32) & (parameter[1] <= 33554432)) // border 2^25 element input
				{
					...
					if (menuChoice == 0)
					{
						...
						AutocorrelationTranSboxGPU_ButterflyMax(device_Sbox, device_CF, device_ACT, sizeSbox, false);
						/////////////////////////////////////////////////////////////////////////////////////////////////
...
void ReadFromFileComputeANF_degSboxGPU(string filename, int menuChoice)
{
	...
	//Declaration and Allocate memory blocks 
	ANF = (int *)malloc(sizeof(int)* size);
	...
	cudaStatus = cudaMalloc((void **)&device_ANF, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed! device_ACT ");
		exit(EXIT_FAILURE);
	}
...
void ReadFromFileComputeBitwiseANF_degSboxGPU(string filename, int menuChoice)
{
	...
	host_CF = (int *)malloc(sizeof(int)* size);
	host_max_values = (int *)malloc(sizeof(int)* size);
	host_NumIntVecCF = (unsigned long long int *)malloc(sizeof(unsigned long long int)* size);
	...
	//@Declaration and Alocation of memory blocks
	int sizeArray = sizeof(unsigned long long int)* size;
	...
	cudaStatus = cudaMalloc((void **)&device_Vec_max_values, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		exit(EXIT_FAILURE);
	}
...
void ReadFromFileComputeDDT_deltaSboxGPU(string filename, int menuChoice)
{
	...
	//Declaration and Allocate memory blocks 
	DDT = (int *)malloc(sizeof(int)* size);
	...
	cudaStatus = cudaMalloc((void **)&device_DDT, sizeof(int)* size);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed! device_ACT ");
		exit(EXIT_FAILURE);
	}
